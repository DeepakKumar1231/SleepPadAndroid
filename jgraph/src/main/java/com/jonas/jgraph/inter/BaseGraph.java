...
public abstract class BaseGraph extends View  {
    private static final String TAG = "BaseGraph.szip";
    /**
     * 选中的 柱状图
     */
...
//     */
     ...
     * 虚线 用移动
     */
    protected float mPhase = 0;

    private int sleepLine = 0;
    ...
    protected void init(Context context){
        ...
        mInterval = MathHelper.dip2px(mContext, 1);//默认的间隔大小
    }
...
//        mGestureDetector = new GestureDetector(mContext, this);
    ...
    protected void refreshChartSetData(){
        if(mGraphStyle == BAR) {
            //柱状图默认 间隔固定
            mInterval = mInterval>=MathHelper.dip2px(mContext, 1) ? mInterval : MathHelper.dip2px(mContext, 1);
            //            mFixBarWidth = false;
        }else {
            //折线图 默认柱子宽度固定 小点
            mBarWidth = 3;
...
//        }else {
        ...
        if(mGraphStyle == BAR) {
            ...
            if (mJcharts!=null)
                ...
                mInterval = ( mChartArea.right-mChartArea.left-mBarWidth*mVisibleNums )/( mVisibleNums-1 );
        }
        refreshExcels();
    }
    ...
    protected void refreshExcels(){
        ...
        for(int i = 0; i<mJcharts.size(); i++) {
            ...
            jchart.setIndex(i);
            if (sleepLine == 2)
                start.x = mChartArea.left+mCharAreaWidth*jchart.getRatio();
            else
        ...
        if (sleepLine == 2)
            mChartRithtest_x = mJcharts.get(mJcharts.size()-1).getMidPointFSleep().x;
        else
            mChartRithtest_x = mJcharts.get(mJcharts.size()-1).getMidPointF().x;
        mChartLeftest_x = mJcharts.get(0).getMidPointF().x;
    }

    public void setSleepLineFlag(int isSleepLine){
        this.sleepLine = isSleepLine;
    }
...
//    }
    ...
    protected void onMyShowPress(MotionEvent e){
        if(BuildConfig.DEBUG) {
            Log.d(TAG, "onShowPress ");
        }
    }
...
//    }
    ...
    protected boolean onMySingleTapUp(MotionEvent e){
        PointF tup = new PointF(e.getX(), e.getY());
        mSelected = clickWhere(tup);
        if(mListener != null) {
            mListener.onItemClick(mSelected);
...
//    }
    ...
    protected boolean onMyScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY){
        return judgeSliding(-distanceX);
    }
...
//    }
    ...
    protected void onMyLongPress(MotionEvent e){
        PointF tup = new PointF(e.getX(), e.getY());
        mSelected = clickWhere(tup);
        if(mListener != null) {
            mListener.onItemLongClick(mSelected);
...
//    }
    ...
    protected boolean onMyFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY){
        upPlace = e2.getX();
        mScroller
                .fling((int)e2.getX(), (int)e2.getY(), (int)velocityX/2, 0, Integer.MIN_VALUE, Integer.MAX_VALUE, 0, 0);
        return true;
    ...
    protected void drawYabscissaMsg(Canvas canvas){
        ...
        for(int i = 0; i<mYaxis_msg.size(); i++) {
            ...
            if(i>0) {
                ...
                if (sleepLine == 1){
                    if (i == mYaxis_msg.size()-1){
                        mAbscisDashPaint.setStrokeWidth(2f);
                        dashPath.moveTo(mChartArea.left, levelCoordinate);
                        dashPath.lineTo(mChartArea.right, levelCoordinate);
                    }else {
                        dashPath.moveTo(mChartArea.left+MathHelper.dip2px(mContext,4), levelCoordinate);
                        dashPath.lineTo(mChartArea.right-MathHelper.dip2px(mContext,4), levelCoordinate);
    ...
    protected void drawCoordinateAxes(Canvas canvas){
        if(widthOne!=mCharAreaWidth) {
            ...
            Path dashPath = new Path();
            dashPath.moveTo(mChartArea.left,mChartArea.bottom);
            dashPath.lineTo(mCharAreaWidth+mChartArea.left,mChartArea.bottom);
            canvas.drawPath(dashPath,mCoordinatePaint);
//            canvas.drawLine(mChartArea.left, mChartArea.bottom, coordinate_x, mChartArea.bottom, mCoordinatePaint);
        }else {
//            mCoordinatePaint.setPathEffect(pathDashEffect(new float[]{15, 5}));
            Path dashPath = new Path();
            dashPath.moveTo(mChartArea.left,mChartArea.bottom);
            dashPath.lineTo(mChartArea.right,mChartArea.bottom);
            canvas.drawPath(dashPath,mCoordinatePaint);
        ...
        for(int i = 0; i<showYnum; i++) {
            if (sleepLine == 2){
                ...
                if (i!=showYnum-1)
                    ...
                    mYaxis_msg.add(String.format("%1dh",(int)(mYaxis_min+diffLevel*i)/60));
            }else {
                mYaxis_msg.add(new DecimalFormat("#").format(mYaxis_min+diffLevel*i));
            }
        }
        mYaxismax = mYaxis_msg.get(showYnum-1);
    }
